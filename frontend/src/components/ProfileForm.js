import React, { useState, useEffect } from "react";
import {
  TextField,
  Button,
  Grid,
  Select,
  MenuItem,
  FormControl,
  InputLabel,
  Typography,
  Checkbox,
  FormControlLabel,
} from "@mui/material";
import AvatarUploader from "./AvatarUploader";

const ProfileForm = ({ userData, onSave }) => {
  const [formData, setFormData] = useState({
    nome: "",
    nomeSocial: "",
    nomeMae: "",
    cpf: "",
    dataNascimento: "",
    idade: "",
    sexo: "",
    telefone: "",
    celular: "",
    email: "",
    cep: "",
    endereco: "",
    numero: "",
    complemento: "",
    bairro: "",
    cidade: "",
    possuiConvenio: "Não",
    estadoCivil: "",
    filhos: "",
    irmaos: "",
    pessoasMorando: "",
    moraCom: "",
    moradia: "",
    linkedin: "",
    facebook: "",
    avatarUrl: "", // Adicionando o campo para o avatar
    ...userData, // Se userData vier com algum dado, ele sobrescreve os valores padrão
  });

  const [hasConvenio, setHasConvenio] = useState(formData.possuiConvenio === "Sim");

  // Atualiza o estado do formulário quando `userData` muda
  useEffect(() => {
    setFormData({
      ...formData,
      ...userData,
    });
    setHasConvenio(userData.possuiConvenio === "Sim");
  }, [userData]);

  const handleChange = (event) => {
    setFormData({
      ...formData,
      [event.target.name]: event.target.value,
    });
  };

  const handleAvatarUpload = (avatarUrl) => {
    setFormData({
      ...formData,
      avatarUrl,
    });
  };

  const handleConvenioChange = (event) => {
    const checked = event.target.checked;
    setHasConvenio(checked);
    setFormData({
      ...formData,
      possuiConvenio: checked ? "Sim" : "Não",
    });
  };

  const handleSubmit = (event) => {
    event.preventDefault();
    onSave(formData); // Chamar a função de salvar
  };

  return (
    <form onSubmit={handleSubmit}>
      <Grid container spacing={2}>
        {/* Upload do Avatar */}
        <Grid item xs={12}>
          <AvatarUploader
            currentAvatar={formData.avatarUrl}
            onUpload={handleAvatarUpload}
          />
        </Grid>

        {/* Dados Pessoais */}
        <Grid item xs={12}>
          <Typography variant="h6">Dados Pessoais</Typography>
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="nome"
            label="Nome Completo"
            value={formData.nome || ""}
            onChange={handleChange}
            required
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="nomeSocial"
            label="Nome Social"
            value={formData.nomeSocial || ""}
            onChange={handleChange}
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="nomeMae"
            label="Nome da Mãe"
            value={formData.nomeMae || ""}
            onChange={handleChange}
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="cpf"
            label="CPF"
            value={formData.cpf || ""}
            onChange={handleChange}
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="dataNascimento"
            label="Data de Nascimento"
            value={formData.dataNascimento || ""}
            onChange={handleChange}
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="idade"
            label="Idade"
            value={formData.idade || ""}
            onChange={handleChange}
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <FormControl fullWidth>
            <InputLabel>Sexo</InputLabel>
            <Select
              name="sexo"
              value={formData.sexo || ""}
              onChange={handleChange}
            >
              <MenuItem value="Masculino">Masculino</MenuItem>
              <MenuItem value="Feminino">Feminino</MenuItem>
              <MenuItem value="Não Informar">Não Informar</MenuItem>
            </Select>
          </FormControl>
        </Grid>

        {/* Contatos */}
        <Grid item xs={12}>
          <Typography variant="h6">Contatos</Typography>
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="telefone"
            label="Telefone"
            value={formData.telefone || ""}
            onChange={handleChange}
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="celular"
            label="Celular"
            value={formData.celular || ""}
            onChange={handleChange}
          />
        </Grid>
        <Grid item xs={12}>
          <TextField
            fullWidth
            name="email"
            label="Email"
            value={formData.email || ""}
            onChange={handleChange}
          />
        </Grid>

        {/* Endereço */}
        <Grid item xs={12}>
          <Typography variant="h6">Endereço</Typography>
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="cep"
            label="CEP"
            value={formData.cep || ""}
            onChange={handleChange}
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="endereco"
            label="Endereço"
            value={formData.endereco || ""}
            onChange={handleChange}
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="numero"
            label="Número"
            value={formData.numero || ""}
            onChange={handleChange}
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="complemento"
            label="Complemento"
            value={formData.complemento || ""}
            onChange={handleChange}
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="bairro"
            label="Bairro"
            value={formData.bairro || ""}
            onChange={handleChange}
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="cidade"
            label="Cidade"
            value={formData.cidade || ""}
            onChange={handleChange}
          />
        </Grid>

        {/* Convenio */}
        <Grid item xs={12}>
          <FormControlLabel
            control={
              <Checkbox
                name="possuiConvenio"
                checked={hasConvenio}
                onChange={handleConvenioChange}
              />
            }
            label="Possui Convênio?"
          />
        </Grid>

        {/* Estado Civil */}
        <Grid item xs={12}>
          <Typography variant="h6">Estado Civil</Typography>
        </Grid>
        <Grid item xs={12} sm={6}>
          <FormControl fullWidth>
            <InputLabel>Estado Civil</InputLabel>
            <Select
              name="estadoCivil"
              value={formData.estadoCivil || ""}
              onChange={handleChange}
            >
              <MenuItem value="Solteiro(a)">Solteiro(a)</MenuItem>
              <MenuItem value="Casado(a)">Casado(a)</MenuItem>
              <MenuItem value="Divorciado(a)">Divorciado(a)</MenuItem>
              <MenuItem value="Viúvo(a)">Viúvo(a)</MenuItem>
            </Select>
          </FormControl>
        </Grid>

        {/* Filhos */}
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="filhos"
            label="Quantos Filhos?"
            value={formData.filhos || ""}
            onChange={handleChange}
          />
        </Grid>

        {/* Irmãos */}
        <Grid item xs={12}>
          <Typography variant="h6">Irmãos</Typography>
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="irmaos"
            label="Tem irmãos? Quantos?"
            value={formData.irmaos || ""}
            onChange={handleChange}
          />
        </Grid>

        {/* Pessoas com quem mora */}
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="pessoasMorando"
            label="Com quantas pessoas mora?"
            value={formData.pessoasMorando || ""}
            onChange={handleChange}
          />
        </Grid>
        <Grid item xs={12}>
          <TextField
            fullWidth
            name="moraCom"
            label="Mora com?"
            value={formData.moraCom || ""}
            onChange={handleChange}
          />
        </Grid>

        {/* Moradia */}
        <Grid item xs={12}>
          <FormControl fullWidth>
            <InputLabel>Moradia</InputLabel>
            <Select
              name="moradia"
              value={formData.moradia || ""}
              onChange={handleChange}
            >
              <MenuItem value="Urbana">Urbana</MenuItem>
              <MenuItem value="Rural">Rural</MenuItem>
            </Select>
          </FormControl>
        </Grid>

        {/* Redes Sociais */}
        <Grid item xs={12}>
          <Typography variant="h6">Redes Sociais</Typography>
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="linkedin"
            label="URL LinkedIn"
            value={formData.linkedin || ""}
            onChange={handleChange}
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            name="facebook"
            label="URL Facebook"
            value={formData.facebook || ""}
            onChange={handleChange}
          />
        </Grid>

        {/* Salvar */}
        <Grid item xs={12}>
          <Button type="submit" variant="contained" color="primary">
            Salvar
          </Button>
        </Grid>
      </Grid>
    </form>
  );
};

export default ProfileForm;
